...
void Huffman::create_codes(HNode* node, const std::string& code) {
    ...
    if(node->left == nullptr && node->right == nullptr){
        //store the node's character and code value in the map
        codes.insert(std::make_pair(node->value, code));
    }

    //if nodes left child exists
    if(node->left != nullptr)
        //traverse left tree
        create_codes(node->left, code + "0");
    //if nodes right child exists
...
// there is no return value as s is being edited with each recursive call
void Huffman::serialize_tree(HNode* node, std::string& s) {
    //if current node is null add a '/' to the string
    if(node == nullptr){
        s += "/";
        return;
    }

    //else store current nodes value and recurse through left and right subtrees
    s += node->value;
...
void Huffman::encode_string(const std::string& input, std::string& encoded_string) {
    //build the encoded string from the map of values
